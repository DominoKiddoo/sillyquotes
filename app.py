...
def generate_image(seed=None):
    ...
    try:
        ...
        person_img = Image.open(BytesIO(img_response.content)).convert("RGBA")

    except requests.exceptions.RequestException as e:
        print(f"Error fetching data: {e}")
        # Create a fallback error image
        canvas = Image.new("RGBA", (1280, 720), "darkred")
        draw = ImageDraw.Draw(canvas)
        try:
            font = ImageFont.truetype("arial.ttf", 40)
        except IOError:
            font = ImageFont.load_default()
        draw.text((50, 300), f"Error: Could not fetch user data.\nPlease try refreshing.", font=font, fill="white")
        img_io = BytesIO()
    ...
    # Step 4: Resize portrait and create fade mask
    person_img = person_img.resize((800, 800))
    fade_mask = Image.new('L', person_img.size, 255)
    for x in range(person_img.width):
        alpha = int(255 * (x / person_img.width))
        for y in range(person_img.height):
            fade_mask.putpixel((x, y), alpha)
    person_img.putalpha(fade_mask)

    # Step 5: Paste the image onto the canvas
    x_pos = width - 800
    y_pos = (height - 800) // 2
    canvas.paste(person_img, (x_pos, y_pos), person_img)

    # Step 6: Draw text with wrapping
    draw = ImageDraw.Draw(canvas)
    quotetext = random.choice(random_sentences)
    if random.randint(1, 2) == 2:
        quotetext = quotetext + " " + random.choice(random_sentences)
    
    draw.text((60, height // 2 - 150), full_name.upper(), font=font_name, fill="red")
    
    y_text = height // 2 - 20
    wrapper = textwrap.TextWrapper(width=30)
    wrapped_quote = wrapper.wrap(text='"' + quotetext + '"')
    
    try:
        line_height = font_quote.getbbox("A")[3]
    except AttributeError:
        _, line_height = font_quote.getsize("A")

    for line in wrapped_quote:
        draw.text((60, y_text), line, font=font_quote, fill="white")
        y_text += line_height + 10

    # Step 7: Save image to a memory buffer
    img_io = BytesIO()
...
def serve_image():
    """
    This endpoint generates the image and sends it directly.
    It now accepts a 'seed' query parameter to make generation deterministic.
    """
    seed = request.args.get('seed')
    image_buffer = generate_image(seed=seed)
    return send_file(image_buffer, mimetype='image/png')

@app.route('/')
def home():
    """
    This is the main homepage. It generates a random seed and passes it
    to the template for both the image and the download link.
    """
    # Generate a random seed for this specific page load.
    ...
    <head>
        ...
        <title>Random Quote Image</title>
        <style>
            body { background-color: #111; margin: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh; font-family: sans-serif; color: white; text-align: center; }
    ...
    <body>
        <div class="container">
            <!-- The seed is added as a query parameter to the image URL -->
            <img src="/image.png?seed={{ seed }}" alt="Randomly generated quote image">
            <div class="button-container">
                <!-- This button reloads the page to get a new image -->
                <a href="/" class="btn new-quote-btn">New Quote</a>
                <!-- This button downloads the currently displayed image -->
                <a href="/image.png?seed={{ seed }}" download="quote-{{ seed }}.png" class="btn download-btn">Download Image</a>
            </div>
        </div>
    </body>
    </html>
